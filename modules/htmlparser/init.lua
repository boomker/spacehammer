...
--[[ local prn = opts.silent and noop
    or function(l, f, ...)
...
local function parse(text, limit) -- {{{
    local options = rine(opts) -- use top-level opts-table (the one, defined before requiring the module), if exists
        or rit(htmlparser_opts) -- or defined after requiring (but before calling `parse`)
        or {} -- fallback otherwise
    options.looplimit = options.looplimit or 500

    local txt_str = str(text)
    local loop_limit = limit or options.looplimit or 1000
    local tpl = false

    if not options.keep_comments then -- Strip (or not) comments {{{
        txt_str = txt_str:gsub("<!%-%-.-%-%->", "") -- Many chances commented code will have syntax errors, that'll lead to parser failures
    end -- }}}

    local tpr = {}

    if not options.keep_danger_placeholders then -- {{{ little speedup by cost of potential parsing breakages
        -- search unused "invalid" bytes {{{
        local busy, i = {}, 0
        repeat -- {{{
            local cc = char(i)
            if not (txt_str:match(cc)) then -- {{{
        ...
        txt_str = txt_str:gsub("(=[%s]-)" .. "(%b'')", function(...) return g(2, ...) end)
            :gsub("(=[%s]-)" .. '(%b"")', function(...)
                return g(2, ...)
            end)
            :gsub(
                "(<" -- Match "<",
                    .. (options.tpl_skip_pattern or "[^!]") -- with exclusion pattern (for example, to ignore comments, which aren't template placeholders, but can legally contain "<"/">" inside.
            ...
            :gsub(
                "("
                    ...
                    .. ")("
                    .. (options.tpl_marker_pattern or "[^%w%s]") -- Capture templating symbol
    ...
    local index = 0
    local root = ElementNode:new(index, str(txt_str))
    local node, descend, tpos, opentags = root, true, 1, {}

    while true do -- MainLoop {{{
        if index == loop_limit then -- {{{
        ...
        while true do -- TagCloseLoop {{{
            ...
            if closingloop == loop_limit then
