...
local function styleText(text)
    return hs.styledtext.new(text, {
        font = {
            name = "Fura Code Retina Nerd Font Complete",
            size = 16
        },
        color = { hex = '#0096FA' },
        paragraphStyle = {
...
local function drawText(tag, renderTextData, ccanvas)
    ...
    for k, v in ipairs(renderTextData) do
        ...
        if math.fmod(k, MAX_LINE_NUM) == 0 then
            ...
            -- 多 text size w 累加
            w = w + size.w
            if k == MAX_LINE_NUM then h = size.h end
    ...
    if tag == 'WindowMOnly' then
        ...
        for _, v in ipairs(winman_keys) do
            if v.tag == "origin" or v.tag == "tile" then
                if #v.prefix == 0 then v.prefix = "⌃⌥⇧ + W + "
                elseif #v.prefix == 1 then v.prefix = "⌃⌥⇧ + W + " .. keyName2Symbol[v.prefix[1]]
                else
                    v.prefix = "⌃⌥⇧ + W + " .. keyName2Symbol[v.prefix[1]] .. keyName2Symbol[v.prefix[2]]
                end
            elseif v.tag == "grid" then
                if #v.prefix == 0 then v.prefix = "⌃⌥⇧ + G + " end
            else
                print(hs.inspect(v))
            end
...
local function closeHotKeyShow() canvass[1]:hide(.3) canvass[2]:hide(.3) end
